...
# Content Security Policy and ingestion domains - Docs

Copy page

# Content Security Policy and ingestion domains - Docs

As [described on MDN](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP): *Content Security Policy (CSP) is an added layer of security that helps to detect and mitigate certain types of attacks, including Cross-Site Scripting (XSS) and data injection attacks. These attacks are used for everything from data theft, to site defacement, to malware distribution.*
...
> **Want to track CSP violations?** To monitor and analyze when your CSP rules are violated, set up [CSP tracking](/docs/csp-tracking.md) to send violation reports to PostHog.

## Content Security Policy directives needed

Depending on how you have installed `posthog-js`, the SDK will dynamically load extra JavaScript assets as necessary. For example, [the snippet](/docs/getting-started/install?tab=snippet.md) is very small and just loads `array.js`. This requires the `script-src` directive.
...
You can [send reports of CSP rule violations](/docs/csp-tracking.md) to PostHog, which is useful for:
...
1.  Follow the instructions to **bundle all required extensions** when [installing PostHog via a package manager](/docs/libraries/js.md#option-2-install-via-package-manager) or [install the snippet](/docs/libraries/js.md#option-1-install-via-snippet).
